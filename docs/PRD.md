...
- 시료 ID는 등록 순서에 따라 자동 부여 (01, 02, 03…)
- 최초 등록 시 재고는 0으로 초기화
...
재고 판단 기준은 **유효 가용 재고**이다.

```
유효 가용 재고 = max(0, 현재 재고 - 생산 대기 중인 동일 시료의 총 주문량)
```

생산 큐에 이미 등록된 동일 시료 주문이 재고를 선점하고 있으므로, 신규 주문의 가용 재고는 기존 대기 주문량을 차감한 값을 기준으로 한다.

- 유효 가용 재고 >= 주문 수량 → 즉시 CONFIRMED 전환, 재고 차감
- 유효 가용 재고 < 주문 수량 → 생산라인 자동 등록 + PRODUCING 전환

### 생산라인

- 생산라인은 단일 라인 (시료를 하나씩 순차 생산)
- 스케줄링: FIFO 방식
- 생산라인 상태: **EMPTY** (대기 작업 없음) / **RUNNING** (처리 중)
  - EMPTY 상태에서 주문이 등록되면 즉시 생산 시작
  - RUNNING 상태에서 신규 주문은 대기 큐 말단에 추가
...
- 생산 진행도는 실제 경과 시간 기반으로 실시간 계산하여 표시 (progress bar + %)
- 경과 시간 >= 총 생산시간 조건 달성 시 자동 완료 처리
- 생산 완료 시 실 생산량(`actualQty`)이 해당 시료 재고에 추가됨
- 생산 완료 시 주문 상태 PRODUCING → CONFIRMED 자동 전환
- 완료 후 다음 대기 작업이 있으면 즉시 생산 시작
